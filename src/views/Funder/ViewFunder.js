...
const ViewFunder = ({ project }) => {
    const [shareUrl, setShareUrl] = useState(false);
    let history = useHistory();
    const { t, i18n } = useTranslation();

    let url_to_share = slugify(project.name, { prefix: config.urls.front+'/funder/show/'+project.id });
                    ...
                    <div className="offer-reactions">
                        <ul className="reactions-box">
                            ...
                            <li className="reaction comments"><i
                                className="uil uil-comment-dots"></i> <span>{project.commentCount} {t('comments')}</span>
                            </li>
